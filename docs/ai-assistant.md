...
O formulário atual não tem campos de alíquotas individuais de ICMS, IPI, PIS/COFINS, DIFAL ou IBS/CBS. Esses dados não são convertidos em carga tributária total. O NCM é uma confirmação da integração Focus NFe e continua sendo escolhido pelo fluxo fiscal existente. O modo completo não estima códigos fiscais nem capacidade produtiva. Carga manual, taxas, prazos ou capital ausentes podem usar cenário neutro identificado em `0`; quantidade mensal não. `expectedMonthlyUnits` só é aceita quando a mensagem contém contexto explícito de mês/mensal e permanece pendente nos demais casos.
