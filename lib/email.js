...
async function sendEmail(config, { to, subject, text, html }) {
  ...
  if (!response.ok) {
    ...
    const providerMessage = typeof payload?.message === "string" ? ` ${payload.message.slice(0, 300)}` : "";
    const error = new Error(`O provedor de e-mail respondeu com status ${response.status}.${providerMessage}`);
