...
Se a resposta não resolver nenhuma pendência, retorne entries vazio. Nunca invente valores ausentes nem apague dados já resolvidos.`;
...
export function buildGeminiGenerateContentRequest(message, clarification) {
  const allowedFields = clarification?.previousAnalysis?.pending?.map(({ field }) => field);
  return {
    systemInstruction: { parts: [{ text: clarification ? `${EXTRACTION_INSTRUCTIONS}\n\n${CLARIFICATION_INSTRUCTIONS}` : EXTRACTION_INSTRUCTIONS }] },
    ...
    async extract(message, clarification) {
      return timeoutRace(config, async (signal) => {
        const response = await fetchImpl(geminiRequestUrl(config.model, ":generateContent"), {
          ...
          redirect: "error",
          signal,
          body: JSON.stringify(buildGeminiGenerateContentRequest(message, clarification)),
        }).catch(() => { throw new AiAssistantError("GEMINI_CONNECTION_ERROR", 503); });
        if (!response.ok) {
