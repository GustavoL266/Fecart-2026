...
function providerFailure(status, payload) {
  ...
  const upstreamCode = typeof payload?.error?.status === "string" && SAFE_UPSTREAM_CODES.has(payload.error.status)
    ? payload.error.status
    : null;
  if (upstreamCode) failure.upstreamCode = upstreamCode;
  const upstreamField = details
    .filter((item) => item?.["@type"] === "type.googleapis.com/google.rpc.BadRequest")
    .flatMap((item) => Array.isArray(item.fieldViolations) ? item.fieldViolations : [])
    .map((item) => item?.field)
    .find((field) => typeof field === "string" && field.length <= 200
      && /^(?:generationConfig|systemInstruction|contents)(?:[.\[\]A-Za-z0-9_-]+)*$/.test(field));
  if (upstreamField) failure.upstreamField = upstreamField;
  return failure;
}
...
export function verifyGeminiModelAccess(config, { fetchImpl = globalThis.fetch } = {}) {
  return timeoutRace(config, async (signal) => {
    ...
    if (!payload.supportedGenerationMethods.includes("generateContent")) {
      ...
      error.upstreamCode = "METHOD_NOT_SUPPORTED";
      throw error;
    }
