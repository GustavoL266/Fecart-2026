...
Os campos obrigatórios de cálculo são: materialCost, wasteRate, packagingCost, deliveryCost, monthlyPayroll, monthlyFixedCosts, expectedMonthlyUnits, taxRate, paymentFeeRate, commissionRate, desiredNetMargin, inventoryDays, receivingDays, paymentDays e monthlyCapitalRate. Retorne também insuranceCost, otherDirectExpenses, discountRate e fixedDiscountAmount, usando zero estimado quando não fizerem sentido. Não estime expectedMonthlyUnits, marketPrice, capacidade produtiva ou contexto fiscal; quantidade mensal exige mês/mensal explícito e permanece pendente quando ausente.
...
const COMPLETE_CLARIFICATION_INSTRUCTIONS = `No modo complete, uma pendência AI_REQUIRED_FIELD_MISSING autoriza estimar esse campo seguindo as regras de estimativa, exceto expectedMonthlyUnits, que nunca deve ser estimado. Quando a única pergunta pendente for especificamente expectedMonthlyUnits, a pergunta controlada do backend já fornece o significado mensal: uma resposta numérica curta como "10", "é 10" ou "é de 10" resolve expectedMonthlyUnits=10. Nesse caso use source="user_provided", copie em evidence o trecho literal da resposta atual que contém o número, use basis="not-applicable" e não use lote. Isso não autoriza copiar uma quantidade de lote do contexto anterior para expectedMonthlyUnits. Para as outras pendências, use a resposta atual para resolver somente o que ela realmente esclarece. Os campos completos e estimados da análise anterior serão preservados pelo backend.`;
