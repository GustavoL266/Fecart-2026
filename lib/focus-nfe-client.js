...
export class FocusNFeClient {
  ...
  async searchNcms(description) {
    ...
    if (cached && cached.expiresAt > Date.now()) {
      this.#logger.info?.("[Fiscal] Cache hit", { provider: "FocusNFe", query });
      return cached.value;
    }
    ...
    // The provider can send up to 50 matches. Limit the presentation only; do not
    // infer, manufacture, or silently choose a fiscal classification.
    const results = parsed.data.slice(0, 10);
    if (this.#cache.size >= this.#cacheMaxEntries) {
      const oldestKey = this.#cache.keys().next().value;
      if (oldestKey) this.#cache.delete(oldestKey);
    }
