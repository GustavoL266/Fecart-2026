...
function safeHttpsUrl(value) {
  try {
    const url = new URL(boundedText(value, 2_048));
...
function sanitizedProduct(value) {
  ...
  if (!price || !id || !title) return null;
  return {
    id, title, price,
    source: boundedText(value.source || "Marketplace", 200), seller: boundedText(value.seller || value.source || "Marketplace", 200),
    currency: boundedText(value.currency || "BRL", 10), category: boundedText(value.category, 120),
    image: safeHttpsUrl(value.image), url: safeHttpsUrl(value.url), consultedAt: boundedText(value.consultedAt, 40),
...
function sanitizedFocusState(fiscalValidation, fiscalContext) {
  ...
  if (!fiscalValidation || typeof fiscalValidation !== "object" || fiscalValidation.status !== "success" || fiscalValidation.source !== "Focus NFe" || fiscalValidation.code !== code) {
    return { status: "idle", ncm: null, source: "", environment: "", checkedAt: "", productNameForNcmSearch: "", unavailable: false };
  }
  const ncm = fiscalValidation.ncm;
  if (!ncm || String(ncm.codigo || "") !== code) return { status: "idle", ncm: null, source: "", environment: "", checkedAt: "", productNameForNcmSearch: "", unavailable: false };
  return {
    status: "success",
    ncm,
    source: "Focus NFe",
    environment: String(fiscalValidation.environment || ""),
    checkedAt: String(fiscalValidation.checkedAt || ""),
    productNameForNcmSearch: String(fiscalValidation.productNameForNcmSearch || "").trim().replace(/\s+/g, " ").slice(0, 120),
    unavailable: false,
  };
}
...
export function authoritativeProductSnapshot(payload) {
  ...
  const calculationData = {
    ...
    summarySemantics: {
      costPrice: "directCost", additionalCosts: "indirectCost + financialCost + fixedSaleFeePerUnit", suggestedPrice: "technicalPrice", profitMargin: "desiredNetMargin", breakEvenPrice: "breakEvenPrice",
    },
  };
  return {
    name: payload.name,
    ...
    // Colunas legadas continuam apenas como resumo compatível; a memória oficial está no JSONB v7.
    costPrice: result.directCost,
    additionalCosts: result.indirectCost + result.financialCost + result.fixedSaleFeePerUnit,
