...
function sanitizedProduct(value) {
  ...
  return {
    id: String(value.id), asin: String(value.asin || value.id), title: String(value.title), price,
    source: String(value.source || "Amazon"), currency: String(value.currency || "BRL"), category: String(value.category || ""),
...
export function marketReferenceFromRequest(market, inputs) {
  ...
  if (rule === "amazon-average" && stats) return { ...base, price: stats.average, source: market?.marketplace || "Amazon", stats };
  if (rule === "amazon-median" && stats) return { ...base, price: stats.median, source: market?.marketplace || "Amazon", stats };
