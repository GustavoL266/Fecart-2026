...
export async function diagnoseFiscalHub({
  ...
  const prefixValid = normalizedApiKey.startsWith("fh_live_");
  logger.info?.(`[FiscalHub] configured=${configured}`);
  logger.info?.(`[FiscalHub] prefixValid=${prefixValid}`);
  ...
  try {
    ...
    logger.info?.(`[FiscalHub] requestStatus=${response.status}`);
    return resultForStatus(response.status);
  } catch (error) {
    const isTimeout = error?.name === "TimeoutError" || error?.name === "AbortError";
    logger.info?.("[FiscalHub] requestStatus=null");
    return {
      configured: true,
      provider: "FiscalHub",
      status: null,
