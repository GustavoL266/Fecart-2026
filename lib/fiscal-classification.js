...
export async function searchFiscalNcms(client, { q, originalQuery = q }, { logger = console, secrets = [] } = {}) {
  ...
  for (const term of fiscalNcmSearchTerms(classification.normalizedQuery)) {
    ...
    for (const ncm of results) {
      ...
      candidates.set(ncm.codigo, { code: ncm.codigo, description: ncm.descricao_completa });
    }
  }
...
export async function confirmFiscalNcm(client, search, code, classificationId) {
  ...
  const ncm = await client.getNcm(code);
  if (ncm.codigo !== code || !isRelevantFiscalNcm(search.normalizedQuery, ncm)) {
  ...
  return { ncm, confirmation: { classificationId, originalQuery: search.originalQuery, normalizedQuery: search.normalizedQuery, code, description: ncm.descricao_completa } };
}
