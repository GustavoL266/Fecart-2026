...
export class FiscalHubError extends Error {
  constructor(message, { code = "FISCALHUB_ERROR", status = 500, retryable = false, upstreamStatus = null } = {}) {
    super(message);
...
export class FiscalHubClient {
  ...
  async request(path, { method = "GET", body } = {}) {
    ...
    for (let attempt = 0; attempt <= this.#maxRetries; attempt += 1) {
      let response;
      try {
        this.#logger.info?.("[Tax] requestStarted=true");
      ...
      this.#logger.info?.(`[Tax] upstreamStatus=${response.status}`);
      if (response.ok) {
        try {
