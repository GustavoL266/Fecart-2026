...
export function getAiAssistantConfig(env = process.env) {
  ...
  if (!apiKey) configurationErrors.push("OPENAI_API_KEY_MISSING");
  if (provider !== "openai") configurationErrors.push("AI_PROVIDER_UNSUPPORTED");
  if (!/^[a-zA-Z0-9._:-]{1,100}$/.test(model)) configurationErrors.push("AI_MODEL_INVALID");
