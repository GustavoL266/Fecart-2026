...
export const FISCALHUB_BASE_URL = "https://api.fiscalhub.com.br";
...
function parseFiscalHubTimeout(value) {
  const timeoutMs = Number.parseInt(value || "10000", 10);
  if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 30_000) {
    throw new Error("FISCALHUB_TIMEOUT_MS deve ser um inteiro entre 100 e 30000.");
  }
  return timeoutMs;
}

export function getFiscalHubConfig(env = process.env) {
  const apiKey = env.FISCALHUB_API_KEY?.trim() || "";
  const companyId = env.FISCALHUB_EMPRESA_ID?.trim() || "";
  return {
    apiKey,
    baseUrl: FISCALHUB_BASE_URL,
    companyConfigured: companyId.length > 0,
    companyId,
    isConfigured: apiKey.length > 0,
    timeoutMs: parseFiscalHubTimeout(env.FISCALHUB_TIMEOUT_MS),
  };
}

export function taxHealth(config) {
  return {
    provider: "FiscalHub",
    configured: config.isConfigured === true,
    companyConfigured: config.companyConfigured === true,
  };
}
