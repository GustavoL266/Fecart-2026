...
export const ncmSearchSchema = z.object({ q: text(120).min(3, "Informe uma descrição para pesquisar NCM.") });

const brazilState = z.string().trim().toUpperCase().regex(
  /^(AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO)$/,
  "Informe uma UF brasileira válida.",
);

export const taxCalculationSchema = z.object({
  ncm: z.string().transform((value) => value.replace(/\D/g, "")).pipe(z.string().regex(/^\d{8}$/, "Informe e confirme um NCM com 8 dígitos.")),
  originState: brazilState,
  destinationState: brazilState,
