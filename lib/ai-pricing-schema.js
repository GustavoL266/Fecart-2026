...
function clarificationTargetsField(clarification, field) {
  const actionablePending = Array.isArray(clarification?.pending)
    ? clarification.pending.filter(({ code }) => code !== "AI_REQUIRED_FIELD_MISSING").map(({ field: pendingField }) => pendingField)
...
function validateGrounding(entry, message, clarification) {
  ...
  if (typeof entry.value === "number" && entry.source === "user_provided") {
    ...
    // the missing basis (for example, "são por unidade"). This exception is
    // restricted to the sole pending field and still requires the same value
    // to occur in the original context.
    const groundedByClarification = clarificationGroundsField(entry, clarification)
...
function isExplicitUnitCost(entry, message, clarification) {
  ...
  const clarificationFields = new Set(Array.isArray(clarification?.pending)
    ? clarification.pending.filter(({ code }) => code !== "AI_REQUIRED_FIELD_MISSING").map(({ field }) => field)
    : clarification?.pendingFields || []);
...
export function validateAiExtraction(raw, message, currentRates = {}, options = {}) {
  ...
  for (const entry of parsed.data.entries) {
    ...
    if (entry.field === "expectedMonthlyUnits" && entry.source === "estimated") continue;
