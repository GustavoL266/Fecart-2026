...
export const AI_OUTPUT_JSON_SCHEMA = {
  type: "object", additionalProperties: false, required: ["entries"], properties: {
    ...
    entries: { type: "array", items: {
      ...
      required: ["field", "value", "evidence", "basis", "certainty", "batchUnits", "batchEvidence", "correctionEvidence"],
      properties: {
...
function validateGrounding(entry, message) {
  ...
  } else if (entry.value !== null && rule.kind === "enum") {
  ...
  } else if (entry.value !== null && !containsText(entry.evidence, entry.value)) return false;
  if (entry.field === "marketQuery" && !/pesquis|procur|busqu|busca|consult/i.test(message)) return false;
  return true;
}

function isExplicitUnitCost(entry, message) {
...
export function validateAiExtraction(raw, message, currentRates = {}) {
  ...
  for (const entry of parsed.data.entries) {
    ...
    if (!validateGrounding(entry, message)) throw new AiAssistantError("AI_INVALID_RESPONSE", 502);
    ...
    const basis = rule.batch && entry.basis === "unknown" && isExplicitUnitCost(entry, message)
      ? "unit" : entry.basis;
    ...
    if (rule.batch) {
      ...
      if (basis === "unit") {
        ...
        if (!isExplicitUnitCost(entry, message)) { addPending("AI_COST_BASIS_UNKNOWN", entry.field); continue; }
      }
    ...
    if (!current || entry.correctionEvidence !== null) {
      groups.set(entry.field, { value: normalizedValue, components: [component] });
  ...
  if (extractedRateFields.length && rateFields.reduce((sum, field) => sum + combinedRates[field], 0) >= 100) {
    extractedRateFields.forEach((field) => { delete fields[field]; groups.delete(field); });
    addPending("AI_RATE_SUM_INVALID", "desiredNetMargin");
  }

  if (!aiFieldsSchema.safeParse(fields).success) throw new AiAssistantError("AI_INVALID_RESPONSE", 502);
