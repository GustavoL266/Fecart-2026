...
function clarificationTargetsField(clarification, field) {
  const pendingFields = new Set(clarification?.pendingFields || []);
    ...
    if (rule.batch) {
      ...
      if (basis === "batch-total") {
        ...
        if (!entry.batchEvidence || !containsLiteralEvidence(message, entry.batchEvidence)
          || !containsNumber(entry.batchEvidence, entry.batchUnits)
          || (!hasBatchContext(entry.batchEvidence) && !clarificationGroundsBatch(entry, options.clarification))) {
      ...
      const property = basis !== "not-applicable" ? "basis" : entry.batchUnits !== null ? "batchUnits" : "batchEvidence";
