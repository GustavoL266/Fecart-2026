...
export const AI_FIELD_RULES = Object.freeze({
  ...
  taxRate: { schema: percentage, label: "Percentual efetivo de impostos sobre a venda", kind: "percent", hint: /carga tribut[aá]ria|tributos? totais?|impostos? totais?|taxa total de impostos/i },
...
function requiredPricingFields(fields = {}) {
  const required = new Set([
    "materialCost", "wasteRate", "packagingCost", "averageOrderFreight", "averageOrderUnits",
    "productionTimeMinutes", "monthlyFixedCosts", "expectedMonthlyUnits", "taxRate", "desiredNetMargin",
    "inventoryDays", "receivingDays", "paymentDays",
  ]);
  if (fields.laborCostMode === "manual") required.add("laborHourlyCost");
  else {
    required.add("monthlyLaborCost");
    required.add("monthlyProductiveHours");
  }
  if (fields.freightPayer === "shared") required.add("companyFreightShare");
  if (fields.capitalRateSource !== "zero") required.add("monthlyCapitalRate");
  if (fields.allocationMethod === "labor-hours" && fields.laborCostMode === "manual") required.add("allocationLaborHours");
  if (fields.allocationMethod === "machine-hours") {
    required.add("machineTimeMinutes");
    required.add("monthlyMachineHours");
  }
  if (fields.allocationMethod === "revenue") {
...
// Structured Output ainda pode representar percentuais como strings apesar da
// instrução. Normalize somente esse formato estreito; todo o restante continua
// sujeito ao schema e às validações de domínio/evidência abaixo.
export function normalizeAiExtraction(raw) {
  ...
  return {
    ...
    entries: raw.entries.map((entry) => {
      ...
      if (AI_FIELD_RULES[entry.field]?.kind === "percent" && typeof entry.value === "string") {
        const match = entry.value.trim().match(/^([+-]?\d+(?:[.,]\d+)?)\s*(?:%|por\s*cento)?$/i);
        if (match) normalizedEntry.value = Number(match[1].replace(",", "."));
      }
...
export function validateAiExtraction(raw, message, currentRates = {}, options = {}) {
  ...
  for (const entry of parsed.data.entries) {
    ...
    if (!validateGrounding(entry, message, options.clarification)) {
      ...
      throw semanticValidationFailure(entry, "evidence", "grounded_literal_string", `invalid_${entry.source}_grounding`);
    }
    ...
    if (!checked.success) {
      if (typeof entry.value !== "number") throw semanticValidationFailure(entry, "value", rule.kind, "invalid_field_value_type");
    ...
    if (rule.batch) {
      if (!["unit", "batch-total", "unknown"].includes(basis)) {
        throw semanticValidationFailure(entry, "basis", "unit_or_batch_total_or_unknown", "invalid_cost_basis");
      }
      if (basis === "unknown") { addPending("AI_COST_BASIS_UNKNOWN", entry.field); continue; }
      if (basis === "unit") {
        if (entry.batchUnits !== null) throw semanticValidationFailure(entry, "batchUnits", "null", "unit_cost_has_batch_units");
        if (entry.batchEvidence !== null) throw semanticValidationFailure(entry, "batchEvidence", "null", "unit_cost_has_batch_evidence");
      ...
      if (basis === "batch-total") {
        ...
        if (!directlyGroundedBatch && !clarificationGroundsBatch(entry, options.clarification)
          ...
          throw semanticValidationFailure(entry, "batchEvidence", "grounded_batch_literal_string", "invalid_batch_grounding");
        }
      }
    } else if (rule.monthly) {
      if (basis !== "monthly-total") throw semanticValidationFailure(entry, "basis", "monthly_total", "invalid_monthly_basis");
      if (entry.batchUnits !== null) throw semanticValidationFailure(entry, "batchUnits", "null", "monthly_cost_has_batch_units");
      if (entry.batchEvidence !== null) throw semanticValidationFailure(entry, "batchEvidence", "null", "monthly_cost_has_batch_evidence");
      ...
      const expectedType = property === "basis" ? "not_applicable" : "null";
      throw semanticValidationFailure(entry, property, expectedType, "invalid_non_cost_metadata");
    }
