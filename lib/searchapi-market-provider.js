...
const SEARCH_RESPONSE_MAX_BYTES = 500_000;
...
const searchResponseSchema = z.object({
  ...
  popular_products: z.array(shoppingResultSchema).optional(),
...
async function readResponsePayload(response) {
  try {
    return await readLimitedJsonResponse(response, { maxBytes: SEARCH_RESPONSE_MAX_BYTES });
  } catch {
    return null;
  }
}
...
export class SearchApiMarketProvider {
  ...
  async #requestSearch(query, page = 1) {
    ...
    try {
      ...
      const products = [...(parsed.data.shopping_results || []), ...(parsed.data.popular_products || [])];
      const seen = new Set();
      const results = products.map((product) => normalizeProduct(product, consultedAt)).filter((item) => {
        if (!item || seen.has(item.id)) return false;
        seen.add(item.id);
        return true;
      });
      this.#logger.info?.(`[Market] Results: ${results.length}`, { received: products.length, page });
      return { results, query, page, received: products.length };
    } finally {
      this.#logger.info?.(`[Market] Duration: ${Math.max(0, Math.round(this.#durationNow() - startedAt))}ms`);
    }
  }

  async #searchFresh(query) {
    const searches = [];
    const first = await this.#requestSearch(query);
    searches.push({ query: first.query, page: first.page, received: first.received, normalized: first.results.length });
    const candidates = [...first.results];
    let ranked = rankMarketResults(candidates, query);

    if (ranked.primaryCount < MIN_PRIMARY_RESULTS) {
      const simplerQuery = simplifyMarketQuery(query);
      const secondQuery = simplerQuery !== query ? simplerQuery : query;
      const secondPage = simplerQuery !== query ? 1 : 2;
      try {
        const second = await this.#requestSearch(secondQuery, secondPage);
        searches.push({ query: second.query, page: second.page, received: second.received, normalized: second.results.length });
        const seen = new Set(candidates.map((item) => item.id));
        for (const item of second.results) {
          if (seen.has(item.id)) continue;
          seen.add(item.id);
          candidates.push(item);
        }
        ranked = rankMarketResults(candidates, query);
      } catch (error) {
        if (!ranked.results.length) throw error;
        this.#logger.warn?.("[Market] Additional search failed", { code: error.code || "SEARCHAPI_ERROR" });
      }
    }

    const results = ranked.results.slice(0, SEARCH_RESULT_LIMIT);
    this.#logger.info?.("[Market] Relevance", { candidates: candidates.length, retained: results.length, primary: ranked.primaryCount, accessories: ranked.accessoryCount });
    return {
      results,
      consultedAt: new Date(this.#now()).toISOString(),
      receivedCount: searches.reduce((sum, search) => sum + search.received, 0),
  ...
  async search(query, { refresh = false } = {}) {
    ...
    if (!refresh && cached && cached.expiresAt > this.#now()) {
      this.#logger.info?.("[Market] Cache: hit");
      return { ...cached.result, cached: true };
    }
    if (cached) this.#searchCache.delete(key);
    if (this.#inflightSearches.has(key)) {
      if (!refresh) return this.#inflightSearches.get(key);
      await this.#inflightSearches.get(key).catch(() => {});
    }

    const request = this.#searchFresh(normalizedQuery)
      .then((response) => {
        const result = { ...response, query: normalizedQuery, marketplace: "Google Shopping", provider: "SearchAPI / Google Shopping" };
        this.#searchCache.set(key, { expiresAt: this.#now() + (result.results.length ? this.#searchCacheTtlMs : EMPTY_CACHE_TTL_MS), result });
