...
export function handleAiRequestError(error, req, res, next) {
  ...
  const safe = invalidBody ? new AiAssistantError("INVALID_AI_REQUEST", error.status) : new AiAssistantError();
  return res.status(safe.status).json({ error: safe.message, code: safe.code });
}

export function createAiPricingRouter({ requireAuth, provider, rateLimitOptions = {} }) {
  const router = Router();
  ...
  router.post("/parse-pricing", requireAuth, userLimiter, ipLimiter, async (req, res) => {
    ...
    try {
      ...
      const safe = error instanceof AiAssistantError ? error : new AiAssistantError();
      return res.status(safe.status).json({ error: safe.message, code: safe.code });
    } finally {
